...
def install_system_dependencies():
    ...
    if sys.platform.startswith("darwin"):  # macOS
        ...
        logger.warning("⚠️  For Windows, please install ffmpeg manually and add it to PATH")
        return True
    ...
    if missing_vars:
        ...
        for var in missing_vars:
            if var == "OPENAI_API_KEY":
                logger.info(f"   {var}=your-openai-api-key  # Get from https://platform.openai.com/")
            else:
...
def main():
    ...
    if success:
        ...
        logger.info("   • WebSocket endpoint: ws://localhost:8000/api/v1/voice-chat/{client_id}")
