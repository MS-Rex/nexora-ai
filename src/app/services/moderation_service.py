...
class ModerationResult(BaseModel):
    ...
    categories: ModerationCategory = Field(..., description="Detailed category breakdown")
    reason: Optional[str] = Field(None, description="Human-readable reason for flagging")
...
class ModerationService:
    ...
    async def moderate_content(self, content: str) -> ModerationResult:
        ...
        try:
            ...
            if self.api_key:
                ...
                logger.warning("OpenAI API key not configured, falling back to keyword filtering")
                return await self._keyword_moderation(content)
                
        except Exception as e:
            logger.error(f"Moderation service error: {e}")
            logger.warning(f"Moderation failed for content (allowing): {content[:100]}...")
    ...
    async def _keyword_moderation(self, content: str) -> ModerationResult:
        ...
        reason = f"Content flagged for: {', '.join(flagged_categories)}" if flagged else None
