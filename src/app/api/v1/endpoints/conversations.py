...
@router.get(
    "/conversations/{session_id}/summary", response_model=ConversationSummaryResponse
)
async def get_conversation_summary(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving conversation summary: {str(e)}"
        )
...
async def get_conversation_history(
    ...
    api_key: Annotated[str, Depends(verify_api_key)],
    limit: int = Query(
        default=50, ge=1, le=100, description="Maximum number of messages to retrieve"
    ),
    db_session: AsyncSession = Depends(get_database_session),
...
async def deactivate_conversation(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error deactivating conversation: {str(e)}"
        )
