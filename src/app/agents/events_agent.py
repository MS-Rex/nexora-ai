...
class EventsAgent:
    ...
    def __init__(self):
        ...
        self.agent_name = agent_prompts_loader.get_agent_name(
            agent_type="events_agent", agent_name="events_agent"
        )
    ...
    def _register_tools(self, agent: Agent[EventsAgentDeps]) -> None:
        ...
        async def fetch_events(
            ctx: RunContext[EventsAgentDeps], event_id: int = 0
        ) -> Dict[str, Any]:
            """
            ...
            try:
                ...
                logger.info(
                    f"Successfully fetched events data: {len(events_data) if isinstance(events_data, list) else 1} events"
                )
        ...
        async def search_events(
            ctx: RunContext[EventsAgentDeps], query: str
        ) -> Dict[str, Any]:
            """
    ...
    async def handle_events_query(
        ...
        try:
            ...
            return agent_prompts_loader.get_error_message(
                "general_error", agent_type="events_agent", agent_name="events_agent"
            )
        finally:
    ...
    async def set_reminder(
        self, event_details: str, user_id: Optional[str] = None
    ) -> str:
        """
