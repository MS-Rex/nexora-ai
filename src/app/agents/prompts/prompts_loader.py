...
class AgentPromptsLoader:
    ...
    def get_agent_config(self, agent_type: str = "chat_agent", agent_name: str = "nexora_ai_assistant") -> Dict[str, Any]:
        """Get configuration for a specific agent."""
        prompts = self.load_agent_prompts(agent_type)
        if agent_name not in prompts:
            raise KeyError(f"Agent '{agent_name}' not found in {agent_type} prompts configuration")
        return prompts[agent_name]
    
    def get_system_instructions(self, agent_type: str = "chat_agent", agent_name: str = "nexora_ai_assistant") -> str:
    ...
    def get_agent_name(self, agent_type: str = "chat_agent", agent_name: str = "nexora_ai_assistant") -> str:
        """Get the display name for an agent."""
        config = self.get_agent_config(agent_type, agent_name)
        return config.get("agent_name", "AI Assistant")
    
    def get_error_message(self, error_type: str = "general_error", agent_type: str = "chat_agent", agent_name: str = "nexora_ai_assistant") -> str:
    ...
    def get_fallback_response(self, response_type: str, agent_type: str = "chat_agent", agent_name: str = "nexora_ai_assistant") -> str:
