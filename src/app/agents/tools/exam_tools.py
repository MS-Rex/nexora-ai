    ...
    async def get_user_exam_results(ctx: RunContext) -> Dict[str, Any]:
        ...
        try:
            ...
            if result["success"]:
                ...
                logger.info(f"Successfully fetched {len(exam_results) if isinstance(exam_results, list) else 'unknown'} exam results for user {user_id_int}")
                return {
                    "user_id": user_id_int,
                ...
                logger.error(f"Failed to fetch exam results for user {user_id_int}: {result.get('error', 'Unknown error')}")
                return result
                
        except Exception as e:
            logger.error(f"Unexpected error in get_user_exam_results for user {ctx.deps.user_id}: {str(e)}")
...
class ExamTools:
    ...
    async def get_user_exam_results_direct(
        ...
        if user_id is None:
            return {"error": "User ID is required to fetch exam results.", "success": False}
