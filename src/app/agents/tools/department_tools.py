...
def register_department_tools(agent: Agent, deps_type: type) -> None:
    ...
    async def fetch_departments(
        ctx: RunContext, department_id: Optional[int] = None
    ) -> Dict[str, Any]:
        """
    ...
    async def search_departments(ctx: RunContext, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Searching departments with query: '{query}', found {len(filtered_departments)} matches"
            )
            return {
                "query": query,
...
class DepartmentTools:
    ...
    async def fetch_departments_direct(
        dependencies: ToolDependencies, department_id: Optional[int] = None
    ) -> Dict[str, Any]:
    ...
    async def search_departments_direct(
        ...
        departments_list = (
            all_departments if isinstance(all_departments, list) else [all_departments]
        )
        filtered_departments = []
        query_lower = query.lower()
