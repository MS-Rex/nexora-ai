...
def register_bus_tools(agent: Agent, deps_type: type) -> None:
    ...
    async def fetch_bus_routes(ctx: RunContext, route_id: int = 0) -> Dict[str, Any]:
        ...
        try:
            ...
            if result["success"]:
                ...
                logger.info(
                    f"Successfully fetched bus routes data: {len(routes_data) if isinstance(routes_data, list) else 1} routes"
                )
                return routes_data
            else:
                return result
        ...
        try:
            ...
            if isinstance(all_routes, list):
                for route in all_routes:
                    ...
                    if (
                        query_lower in route_name
                        or query_lower in route_number
                        or query_lower in start_point
                        or query_lower in end_point
                    ):
                        filtered_routes.append(route)

            logger.info(
                f"Searching bus routes with query: '{query}', found {len(filtered_routes)} matches"
            )
            return {
                "query": query,
    ...
    async def get_routes_by_status(ctx: RunContext, status: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Filtering bus routes by status: '{status}', found {len(filtered_routes)} matches"
            )
            return {
                "status": status,
                "routes": filtered_routes,
    ...
    @agent.tool
    async def get_routes_by_time_range(
        ctx: RunContext, start_time: str, end_time: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(
                f"Filtering bus routes by time range: {start_time}-{end_time}, found {len(filtered_routes)} matches"
            )
            return {
...
class BusTools:
    ...
    async def fetch_bus_routes_direct(
        dependencies: ToolDependencies, route_id: int = 0
    ) -> Dict[str, Any]:
    ...
    async def search_bus_routes_direct(
        dependencies: ToolDependencies, query: str
    ) -> Dict[str, Any]:
