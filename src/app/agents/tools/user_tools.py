    ...
    async def get_user_data(ctx: RunContext) -> Dict[str, Any]:
        ...
        try:
            ...
            if result["success"]:
                ...
                return {"user_id": user_id_int, "user_data": user_data, "success": True}
            else:
                logger.error(
                    f"Failed to fetch user data for user {user_id_int}: {result.get('error', 'Unknown error')}"
                )
                return result

        except Exception as e:
            logger.error(
                f"Unexpected error in get_user_data for user {ctx.deps.user_id}: {str(e)}"
            )
...
class UserTools:
    ...
    async def get_user_data_direct(
        ...
        if user_id is None:
            return {
                "error": "User ID is required to fetch user data.",
                "success": False,
            }
        ...
        if result["success"]:
            return {"user_id": user_id, "user_data": result["data"], "success": True}
        else:
            return result
