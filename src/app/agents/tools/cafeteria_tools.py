...
def register_cafeteria_tools(agent: Agent, deps_type: type) -> None:
    ...
    @agent.tool
    async def fetch_cafeteria_menu(ctx: RunContext, cafeteria_id: int = 0) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if result["success"]:
                ...
                logger.info(f"Successfully fetched cafeteria menu data for cafeteria {cafeteria_id}")
                return menu_data
            else:
                return result
    ...
    @agent.tool
    async def search_menu_items(ctx: RunContext, query: str, cafeteria_id: int = 0) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(f"Searching menu items with query: '{query}', found {len(filtered_items)} matches")
            return {
                "query": query,
...
class CafeteriaTools:
    ...
    async def search_menu_items_direct(
        ...
        menu_data = await CafeteriaTools.fetch_cafeteria_menu_direct(dependencies, cafeteria_id)
